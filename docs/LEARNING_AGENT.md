...
├── saucedemo_evaluation_cases.md # 评测用例文档（10 个用例）
└── evaluation_report.md          # 评测报告（运行后生成）
```
...
## 用例结果
| 用例 | 名称 | 难度 | 得分 | 通过 | 耗时(s) | 步骤数 | 评估摘要 |
|------|------|------|------|------|---------|--------|----------|
| 1 | 成功登录 | 简单 | 100 | ✅ | 12.3 | 3 | 登录成功，正确报告商品数量 |
| 2 | 登录失败 | 简单 | 90 | ✅ | 8.1 | 2 | 正确识别失败并提取错误信息 |
| ... | ... | ... | ... | ... | ... | ... | ... |
```

### 10.7 评测设计要点

- **独立运行**：每个用例运行前清除浏览器状态，避免登录态干扰
- **LLM 评估**：使用 LLM 对照预期结果评估，比硬编码规则更灵活
- **Trace 记录**：每个用例的 trace 文件路径记录在报告中，便于事后分析
- **架构对比**：可通过 `--single` 参数对比单 Agent 与三 Agent 架构的表现
